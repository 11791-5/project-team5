...
public class AnswerExtractor 
{
  ...
  public static Map<Integer,String> getAnswers(List<String> snippets,List<String> queryTerms)
  {
    ...
    for(String snippet:snippets)
    {
      ...
      edu.stanford.nlp.pipeline.Annotation ann = new edu.stanford.nlp.pipeline.Annotation(snippetText);
      ...
      for (CoreLabel term : ann.get(TokensAnnotation.class)) 
      {
        ...
        if(pos.contains("NN") && !StopWordSingleton.getInstance().isStopWord(token) && !queryTerms.contains(token))
        {
  ...
  private static Map<Integer,String> getTopAnswers(TreeMap<String, Integer> rankedPotentialAnswers) 
  {
    ...
    int rank = 1;
    for(String answer: rankedPotentialAnswers.keySet())
    {
      if(rankedPotentialAnswers.get(answer)>threhold)
        topAnswers.put(rank++,answer);
    }
    return topAnswers;
  }
